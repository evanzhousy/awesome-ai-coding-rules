...
> **Data Reference**: [Data Schema](./data_schema.md)  
> **Last Updated**: 2026-01-20

This document summarizes key concepts from the OptionData product documentation for internal reference. It is organized into foundational concepts, core Greeks & metrics, market structure indicators, and real-time flow analysis. **Field names referenced here (e.g., `side`, `sentiment`, `dex`) correspond to the actual data fields documented in [Data Schema](./data_schema.md).**
...
3. [Core Greeks & Metrics](#core-greeks--metrics)
   ...
   - [Delta OI Impact (ΔOI DEI)](#delta-oi-impact-δoi-dei)
...
### Delta OI Impact (ΔOI DEI)

#### Definition
In Contract-level analysis, the **ΔOI DEI** column shows OI Impact: signed day-over-day open-interest change converted into delta-equivalent shares, then scaled by the effective DEI denominator for that option root. For stocks and ETFs, this denominator is the underlying's average stock volume. For supported cash-settled index roots, Contract-level analysis uses a configured ETF proxy denominator so DEI remains liquidity-relative: `SPX` / `SPXW` use `SPY.average_stock_volume / 10`, `XSP` uses `SPY.average_stock_volume`, `NANOS` uses `SPY.average_stock_volume × 100`, `RUT` / `RUTW` use `IWM.average_stock_volume / 10`, `MRUT` uses `IWM.average_stock_volume`, `NDX` / `NDXP` use `QQQ.average_stock_volume / 40`, `XND` uses `QQQ.average_stock_volume × 2.5`, `MGTN` / `MGTNW` use `QQQ.average_stock_volume × 1.5`, `DJX` / `DJXW` use `DIA.average_stock_volume`, `VIX` / `VIXW` use `VXX.average_stock_volume`, `MXEA` uses `EFA.average_stock_volume / 30`, `CBTX` / `CBTXW` use `IBIT.average_stock_volume / 40`, and `MBTX` / `MBTXW` use `IBIT.average_stock_volume / 4`. Other index families remain unknown until a proxy denominator is explicitly defined. It connects the liquidity-relative DEI concept above with Open Interest confirmation from the chain.

#### Formula
```text
ΔOI DEX = ΔOI × 100 × delta
ΔOI DEI = ΔOI DEX ÷ effective DEI denominator × 100
```

#### Interpretation

| ΔOI DEI Value | Meaning |
|---------------|---------|
| **Positive** | Bullish underlying impact |
| **Negative** | Bearish underlying impact |
| **Larger absolute value** | Larger OI-confirmed positioning impact relative to underlying liquidity |
| **Unknown / null** | `oiChange`, `delta`, or a positive effective DEI denominator is missing |

#### Example
If a contract adds **2,000 OI**, has **0.50 delta**, and the underlying's average stock volume is **10,000,000 shares**:

```text
ΔOI DEX = 2,000 × 100 × 0.50 = 100,000 shares
ΔOI DEI = 100,000 ÷ 10,000,000 × 100 = 1.0%
```

In the Contract-level analysis table, this appears as **ΔOI DEI = 1.0%**, and the OI Impact KPI treats it as bullish because the value is positive.
