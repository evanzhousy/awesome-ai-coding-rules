...
Latest read-only preflight run: PostHog, Better Stack, ClickHouse, Massive, Cloudflare/Wrangler, Stripe test mode, Clerk test key, Neon, and Discord env presence were checked. Browser plugin control was not exposed in the current loaded tool set; load or connect the Browser plugin before running browser-dependent runbooks.
...
- stripe/clerk/neon/cloudflare/massive/discord: connected|blocked|skipped; path=<env|browser|wrangler|sdk>; blocker=<none or exact error>
```
