...
- This review-only boundary overrides mutation-oriented steps in companion journey maps. In particular, do not execute the temporary create/rename/duplicate/default/delete/Use/Apply lifecycle from `browser-e2e-product-review.md` without that separate authorization. For an unauthorized Saved Filter round, inventory the controls, use only an unapplied draft, prove discard/recovery, and leave mutation/persistence capabilities explicitly untested.
- When export itself is the scoped read-only user job, one Browser-managed download dispatch is allowed without choosing a filesystem destination. Do not open OS Downloads, inspect unrelated local files, retain or commit the artifact, or treat an unobserved file as evidence. Inspect filename/size/content only when Browser exposes it directly; otherwise prove the button/gate/feedback and mark artifact verification Browser-capability blocked. This exception does not authorize any repository file write beyond the report.
...
5. Record visible before/action/after evidence. Use a screenshot when layout, hierarchy, chart readability, clipping, or mobile fit is the claim; use DOM/state evidence for copy, role, enabled state, route, and settled results. Console/network evidence may corroborate visible breakage but does not replace it. If `Page.captureScreenshot` times out twice while ordinary Browser interaction and DOM inspection remain healthy, make at most one recovery attempt in a fresh Browser tab. If that also fails, record the exact capture blocker, collect precise rectangles plus `clientWidth`, `scrollWidth`, and overflow state as non-visual evidence, and continue only for claims that evidence can support. Never relabel DOM geometry as screenshot or visual proof.
   - For an emulated mobile slice, allow at most one reset → set → reload setup retry. Once Browser reports the target dimensions, verify dimensions read-only immediately before each counted action; do not reissue the viewport setter merely to verify them because a redundant set can restore the prior dimensions. Treat acceptance as tab-specific until proved otherwise: a newly created blank tab may open at default desktop dimensions, so inspect it before navigation. If the active tab drifts or a required fresh tab does not inherit the target dimensions, stop that slice and record a Browser-capability blocker rather than reissuing the setter, substituting reload for fresh entry, or counting desktop geometry as mobile evidence.
   - An overlay in the DOM or accessibility tree is not proof that a sighted user can operate it. If its trigger says expanded but the overlay is transparent, off-viewport, inert, or otherwise contradicts its semantic state, record computed opacity/visibility/pointer state, geometry, focus, and hit testing. Perform this visibility check before the first child interaction in every newly opened authentication, access, billing, or account overlay; do not assume a different overlay works because its semantic controls exist. Try one ordinary recovery such as Escape or the visible Close action, then reload if needed. Do not operate invisible controls through locators as though a novice could see them; mark the covered journey blocked and inspect source only after the Browser evidence is complete. If account restoration or other required cleanup cannot be completed without bypassing an invisible overlay, label that interaction as a cleanup-only exception, exclude it from journey success, and record the exact state.
