...
5. Record visible before/action/after evidence. Use a screenshot when layout, hierarchy, chart readability, clipping, or mobile fit is the claim; use DOM/state evidence for copy, role, enabled state, route, and settled results. Console/network evidence may corroborate visible breakage but does not replace it. If `Page.captureScreenshot` times out twice while ordinary Browser interaction and DOM inspection remain healthy, make at most one recovery attempt in a fresh Browser tab. If that also fails, record the exact capture blocker, collect precise rectangles plus `clientWidth`, `scrollWidth`, and overflow state as non-visual evidence, and continue only for claims that evidence can support. Never relabel DOM geometry as screenshot or visual proof.
   - An overlay in the DOM or accessibility tree is not proof that a sighted user can operate it. If its trigger says expanded but the overlay is transparent, off-viewport, inert, or otherwise contradicts its semantic state, record computed opacity/visibility/pointer state, geometry, focus, and hit testing. Try one ordinary recovery such as Escape or the visible Close action, then reload if needed. Do not operate invisible controls through locators as though a novice could see them; mark the covered journey blocked and inspect source only after the Browser evidence is complete.
