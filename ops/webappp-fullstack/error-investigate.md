...
Active backlog is not empty. Start every real run with [Active fix verification backlog](#active-fix-verification-backlog); current unconfirmed rows include Contract Rank direct snapshot fallback diagnostics, Option Trades Live buffer-overflow summary telemetry, and preference server-function load normalization. Do not archive a row until a post-deploy run clears its listed signals.
...
| --- | --- | --- | --- | --- |
| **Contract Rank direct snapshot recovered-fallback diagnostics** — `contractFlowRankService` splits direct JSON parse vs fetch/decode stages and marks direct snapshot/meta fallbacks with valid `failureKind=transient` plus `fallbackOutcome=recovered_fallback` / `fallbackTarget`, while preserving P1 capture and server fallback behavior. | `tradingflow-webapp-fullstack` local patch, pending deploy, updated 2026-06-18 | 2026-06-17 pre-fix: PostHog issue `019eb693-5781-7bd3-8a42-405e7854ce61` “CF snapshot returned unsupported compact payload” = 4 occurrences / 1 user; Better Stack pattern `dde126aeb830b9ee827a61e475f77e364c2aeed30f53a2cdde735e0688bef63c` = 2 exceptions; JSON parse issue `019e8dfa-d541-7132-a2cf-8a67347950c5` and Better Stack pattern `e32fc0da3a7021e0f7e3205f3bbaaf3e864a73806e8e330cd45c58007347299e` = 3-6 events; Better Stack Info `contractFlowRank.snapshot` direct snapshot failed `error.P1` = 7 events; `query:contractFlowRankSnapshot` unsupported payload `error.P0` = 2 events. | Query PostHog issues `019eb693-5781-7bd3-8a42-405e7854ce61` and `019e8dfa-d541-7132-a2cf-8a67347950c5`; Better Stack patterns `dde126aeb830b9ee827a61e475f77e364c2aeed30f53a2cdde735e0688bef63c` and `e32fc0da3a7021e0f7e3205f3bbaaf3e864a73806e8e330cd45c58007347299e`; Better Stack Info for `scope=contractFlowRank.snapshot`, `failureKind=transient`, `fallbackOutcome=recovered_fallback`, `fallbackTarget in (server_snapshot, server_meta)`, `requestStage in (cf_direct_snapshot_parse, cf_direct_snapshot_decode, cf_direct_snapshot_meta_parse, cf_direct_snapshot_meta_decode)`, and confirm no paired `query:contractFlowRankSnapshot` P0 for recovered direct fallbacks. | No |
| **Option Trades Live buffer-overflow summary telemetry** — `useLiveMode` keeps live pending-buffer drops captured as P1 info but aggregates per-tick overflow into summary events with `overflowEventCount`, per-stage drop counts, and flush-on-close so backpressure is not silent and Better Stack is not flooded. | `tradingflow-webapp-fullstack` local patch, pending deploy, 2026-06-18 | 2026-06-18 pre-fix: Better Stack Info `scope=useLiveMode buffer overflow` / `operation=live_mode_buffer_overflow` emitted 1,822 `pending_buffer_limit` events and 537 `drip_ui_pause` events in the last `-24h` window. | Query Better Stack Info for `scope=useLiveMode buffer overflow` / `operation=live_mode_buffer_overflow`; confirm events use message `Dropped live buffer overflow summary; pending rows bounded`, include `overflowEventCount`, `droppedCount`, `dripPausedDropCount`, and `pendingLimitDropCount`, and total event volume drops materially versus the 2,359-event pre-fix baseline while drops remain represented. | No |
| **Preference server-function load normalization** — `src/services/preference.ts` loads preference/watchlist Saved Filter server functions through `loadNamedServerFunction`, and `src/platform/server-function-loader.ts` wraps import/export-access failures as `ServerFunctionLoadError` so missing or stale chunks report structured `surface=preferences`, `requestStage=server_fn_load`, `failureKind=transient`, `serverModule`, and `serverExport` instead of raw destructuring exceptions. | `tradingflow-webapp-fullstack` local patch, pending deploy, 2026-06-18 | 2026-06-18 pre-fix: PostHog issue `019ed88f-78d6-7a21-93ac-eca1de351b9f` “Cannot destructure property 'getWatchlist'…” = 1 event; PostHog issue `019ed88f-78d6-7a21-93ac-ecb053e91fc1` “Cannot destructure property 'getOptionTradesPreference'…” = 1 event; related Symbol Catalog loader issue `019ed82b-ed18-7083-85e2-ae56b61d4172` “Cannot destructure property 'getSymbolCatalog'…” = 1 event; Better Stack pattern `90e2c3d8ac0bf0e6d5519be61fbb3f26418803fc000d4284528552996824e468` = 3 exceptions for `getWatchlist`; Better Stack pattern `2f018277185b0c3222a4a711d85f184d7b57bd00e6ebb3a1108d8e7ac1367255` = 1 exception for `getSymbolCatalog`. | Query the three PostHog issues above and the two Better Stack patterns above; search Better Stack Errors for messages containing `Cannot destructure property 'getWatchlist'`, `getOptionTradesPreference`, or `getSymbolCatalog`; Better Stack Info should show any remaining load failures as `surface=preferences`, `requestStage=server_fn_load`, `failureKind=transient`, `serverExport in (getWatchlist, getOptionTradesPreference, updateWatchlist, saveOptionTradesPreference, createOptionTradesSavedFilterSet, renameOptionTradesSavedFilterSet, setDefaultOptionTradesSavedFilterSet, duplicateOptionTradesSavedFilterSet, deleteOptionTradesSavedFilterSet)` rather than destructuring messages. | No |
