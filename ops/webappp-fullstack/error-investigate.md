...
Active backlog is not empty. Start every real run with [Active fix verification backlog](#active-fix-verification-backlog); current unconfirmed rows include Option Trades row-timeout serialization, Contract Rank direct snapshot fallback diagnostics, and Option Trades Live buffer-overflow summary telemetry. Do not archive a row until a post-deploy run clears its listed signals.
