...
Active backlog is not empty. Start every real run with [Active fix verification backlog](#active-fix-verification-backlog); current unconfirmed rows include Contract Rank direct snapshot fallback diagnostics and Option Trades Live buffer-overflow summary telemetry. Do not archive a row until a post-deploy run clears its listed signals. Latest implementation handoff: on 2026-06-22 the live CF snapshot endpoint returned valid `contract_rank_snapshot_compact_v9` shape (`rc=397625`, first tuple length `19`, valid `u`/`g` context maps), while production PostHog/Better Stack still pointed at deployed asset `/assets/contract-rank.lazy-HJ_C0EY8.js`; after the next webapp deploy, verify the old asset stops emitting before treating the Contract Rank row as fixed.
...
| --- | --- | --- | --- | --- |
| **Contract Rank direct snapshot recovered-fallback diagnostics** — `contractFlowRankService` splits direct JSON parse vs fetch/decode stages and marks direct snapshot/meta fallbacks with valid `failureKind=transient` plus `fallbackOutcome=recovered_fallback` / `fallbackTarget`, while preserving P1 capture and server fallback behavior. | `tradingflow-webapp-fullstack` local patch, pending webapp deploy, updated 2026-06-22; current checkout accepts the live `contract_rank_snapshot_compact_v9` direct payload, so remaining evidence is deploy skew until post-deploy signals say otherwise. | 2026-06-17 pre-fix: PostHog issue `019eb693-5781-7bd3-8a42-405e7854ce61` “CF snapshot returned unsupported compact payload” = 4 occurrences / 1 user; Better Stack pattern `dde126aeb830b9ee827a61e475f77e364c2aeed30f53a2cdde735e0688bef63c` = 2 exceptions; JSON parse issue `019e8dfa-d541-7132-a2cf-8a67347950c5` and Better Stack pattern `e32fc0da3a7021e0f7e3205f3bbaaf3e864a73806e8e330cd45c58007347299e` = 3-6 events; Better Stack Info `contractFlowRank.snapshot` direct snapshot failed `error.P1` = 7 events; `query:contractFlowRankSnapshot` unsupported payload `error.P0` = 2 events. 2026-06-22 pending-deploy evidence: live direct snapshot probe returned `v=contract_rank_snapshot_compact_v9`, `rc=397625`, first tuple length `19`; PostHog issue `019eb693-5781-7bd3-8a42-405e7854ce61` still active with 2 occurrences / 1 user / 1 session from source `/assets/contract-rank.lazy-HJ_C0EY8.js`; Better Stack pattern `dde126aeb830b9ee827a61e475f77e364c2aeed30f53a2cdde735e0688bef63c` still had 1 production exception from the same asset. | After deploy, first confirm the app is no longer serving `/assets/contract-rank.lazy-HJ_C0EY8.js`, then query PostHog issues `019eb693-5781-7bd3-8a42-405e7854ce61` and `019e8dfa-d541-7132-a2cf-8a67347950c5`; Better Stack patterns `dde126aeb830b9ee827a61e475f77e364c2aeed30f53a2cdde735e0688bef63c` and `e32fc0da3a7021e0f7e3205f3bbaaf3e864a73806e8e330cd45c58007347299e`; Better Stack Info for `scope=contractFlowRank.snapshot`, `failureKind=transient`, `fallbackOutcome=recovered_fallback`, `fallbackTarget in (server_snapshot, server_meta)`, `requestStage in (cf_direct_snapshot_parse, cf_direct_snapshot_decode, cf_direct_snapshot_meta_parse, cf_direct_snapshot_meta_decode)`, and confirm no paired `query:contractFlowRankSnapshot` P0 for recovered direct fallbacks. Sanity-probe the producer shape with `curl -sS --max-time 20 https://ws.optiondata.io/api/v1/contract-rank/latest-snapshot` and confirm `v=contract_rank_snapshot_compact_v9`, `rc == r.length`, and the first tuple length is `19`; if producer shape is valid and old asset emissions continue, treat it as a failed webapp deploy/cache invalidation rather than a CF worker payload bug. | No |
