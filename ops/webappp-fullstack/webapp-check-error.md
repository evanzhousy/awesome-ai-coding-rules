...
Active backlog is not empty. Start every real run with [Active fix verification backlog](#active-fix-verification-backlog); current unconfirmed rows are Contract Rank direct snapshot/server fallback verification and Netlify missing-asset 404 handling. Do not archive the Contract Rank row until a post-deploy run clears both the direct CF fallback signals and the server fallback ClickHouse type-supertype signals. Latest verification handoff: on 2026-06-24 the live CF snapshot endpoint returned valid `contract_rank_snapshot_compact_v9` shape (`rc=349795`, `rc == r.length`, first tuple length `19`, valid `u`/`g` context maps), and the old unsupported-payload issue/patterns were zero in `-24h`, but Better Stack still showed `contractFlowRank.snapshot` P0s from ClickHouse `symbol_context AS sc` numeric type mismatch. The current webapp checkout has a local `Float64` cast patch in `src/server/services/contractFlowRankQueryService.ts`; after deploy, verify those Better Stack patterns drop before archiving the row. The current webapp checkout also has a Netlify `/assets/*` 404 rule plus `public/404.html`; after deploy, verify stale hashed asset URLs return HTTP 404 instead of `200 text/html` before archiving that row. Option Trades Live buffer-overflow summary telemetry was confirmed and archived on 2026-06-24.
