...
Active backlog is not empty. Start every real run with [Active fix verification backlog](#active-fix-verification-backlog); current unconfirmed rows are Contract Rank direct snapshot/server fallback verification and Netlify missing-asset 404 handling. 2026-06-26 greenfield run evidence: production served `0.2.1+1d37b23` built `2026-06-25T18:51:15Z`; the live CF snapshot endpoint returned valid `contract_rank_snapshot_compact_v9` shape (`rc=340446`, `rc == r.length`, first tuple length `19`, valid `u`/`g` maps, 43.4 MB payload); stale asset URLs, including old Contract Rank/Cookbooks chunks, now returned HTTP 404 instead of `200 text/html`; and the prior `NO_COMMON_TYPE` / `symbol_context AS sc` patterns were absent in `-24h`. Do not archive the Contract Rank row yet: direct snapshot producer health is good, but unauthenticated/default-preview server fallback still hit premium-only `Authentication required` patterns (`7d711...` = 4, `528a03...` = 2, `a86c...` = 1) after recovered direct-snapshot fallback attempts; the likely next fix is a preview-aware Contract Rank snapshot server fallback or a guest path that avoids premium-only server fallback. Do not archive the Netlify row yet: asset probes pass and old module/MIME patterns were zero, but `network error` pattern `2403c9...` was still 48 in `-24h`, with last seen around the latest deploy, so wait for a full clean post-deploy window before moving the row to resolved. Net-new P0 cluster from this run: Cookbooks `relation "recipes" does not exist` (`dd377...` NeonDbError = 6, `f82f...` ServerHttpError = 3; `recipes.listMine`/`query:my-recipes`/`serverFn.uncaught` P0 logs) points to production Neon migration drift for `src/server/db/migrations/0001_recipes.sql` + `0002_recipe_forked_from.sql`; verify with `pnpm db:migrate` / `scripts/db-migrate.mjs` against production and confirm the matching Better Stack patterns/log scopes drop after deployment.
