...
Active backlog is not empty. Start every real run with [Active fix verification backlog](#active-fix-verification-backlog); current unconfirmed rows remain Contract Rank direct snapshot/server fallback verification and Netlify missing-asset 404 handling. 2026-06-28 run evidence: production still served `0.2.1+1d37b23` built `2026-06-25T18:51:15Z`; the prior local fix commit `2f12dabe1` was present in local branches but was not reflected in production. The live CF snapshot endpoint returned valid `contract_rank_snapshot_compact_v9` shape (`d=2026-06-26`, `rc=365014`, `rc == r.length`, first tuple length `19`, valid `u`/`g` maps, 46.5 MB payload). The previous `Authentication required`, `NO_COMMON_TYPE`, and Cookbooks `relation "recipes" does not exist` clusters were absent in the current `-24h`; historical `-7d` still showed their old tails with last buckets on 2026-06-26 or earlier, so do not confuse quiet traffic with confirmed deploy. Do not archive the Contract Rank row yet: production is still on the old build and Better Stack Info still showed `contractFlowRank.snapshot` P1 recovered fallback plus one `query:contractFlowRankSnapshot` P0 in the current window. Do not archive the Netlify row yet: stale `/assets/*` probes return 404 instead of SPA `200 text/html`, but PostHog issue `019d2051-6a33-7b10-bb1e-54fa08bb3be7` still had 26 occurrences / 4 users / 12 sessions and Better Stack still showed chunk/load/network tails, so wait for a full clean post-deploy window before moving the row to resolved. Net-new active cluster from this run: Option Trades Live/Historical rows and live transport. Better Stack Telemetry showed `fetchOptionTradesRows` P0 timeouts = 5, `fetchOptionTradesStats` P1 timeouts = 2, and `option-trades-live` P1 transport failures (`network error` = 31, `Load failed` = 14, `Load failed (ws.optiondata.io)` = 4). PostHog aligned on `/app/option-trades/live` stale/load issues and one stale-main-bundle route-context crash. Likely next fix is Option Trades timeout/transport classification and retry hardening: correlate client `elapsedMs` vs `clientTimeoutMs`, `document.visibilityState`, and server-side slow/error logs before tuning queries, because several mobile Safari timer callbacks fired minutes after their nominal 22s/25s timeouts and had no matching server-side slow-success/error rows.
