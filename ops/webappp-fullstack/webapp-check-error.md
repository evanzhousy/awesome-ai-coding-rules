...
Active backlog is not empty. Start every real run with [Active fix verification backlog](#active-fix-verification-backlog); current unconfirmed rows remain Contract Rank direct snapshot/server fallback verification, Netlify missing-asset 404 handling, and Option Trades access-tier/timeout diagnostics. 2026-06-30 rerun evidence after the Option Trades fix was merged into local `test`: local `test` HEAD `ee8ab2593` includes `fcfa6e489`, but production still served `0.2.1+b85fabd` built `2026-06-29T14:53:01.260Z`, so the Option Trades fix is not deployed and cannot be confirmed. Stale asset probes for `/assets/gamma-exposure-screener.lazy-DI28ZLQ0.js`, `/assets/index-D5u4BulH.js`, `/assets/cookbooks.index-GFu0Bycw.js`, and `/assets/contract-rank.lazy-HJ_C0EY8.js` returned `404 text/html`, so the Netlify routing behavior is deployed; do not archive the Netlify row yet because PostHog issue `019d2051-6a33-7b10-bb1e-54fa08bb3be7` still had a tail (7 occurrences / 4 users / 6 sessions) and Better Stack pattern `2403c9ba5deee2af387834e95461cfb32b9b5080b21d6f307b2f09bb09e71f21` still showed `network error` = 18 in `-24h`. The live CF snapshot endpoint returned valid `contract_rank_snapshot_compact_v9` shape (`d=2026-06-29`, `rc=349226`, `rc == r.length`, first tuple length `19`, valid `u`/`g` maps, 45.6 MB payload); sampled Contract Rank PostHog events point at stale `/assets/contract-rank.lazy-HJ_C0EY8.js` 404s rather than a bad producer payload, but do not archive the Contract Rank row yet because PostHog issue `019eb693-5781-7bd3-8a42-405e7854ce61` still had 2 occurrences / 1 user / 1 session and Better Stack Info still showed `contractFlowRank.snapshot` P1 and `query:contractFlowRankSnapshot` P0 failures. Option Trades evidence remains pre-deploy: Better Stack pattern `40f926ceec27f0789c6f75b5997b85585f3c8784fb9f59d2d1180f13df6a08b8` and access-guard `Paid access required` logs were 0 in `-24h`, but rows timeout signals remain (`fetchOptionTradesRows` P0 `timeout_client` logs = 6; Better Stack pattern `358ea88303e1eb9fc5ad2b06c1b7cfceff979e4da75c75b1b85d1c928c3aea6f` = 3; PostHog issue `019f11d7-6eff-7b02-b547-fdd21dff8478` = 4 occurrences / 1 user / 1 session, sampled value `Rows request timed out after 22s` on `/app/rank/contracts`). Net-new active cluster to watch: PostHog issue `019e583e-154f-7f83-9807-71b7d8bcbbca` (`[tv] invalid symbol`) had 7 occurrences / 3 users / 3 sessions on `/app/option-trades/live`; next fix plan should decide whether to guard/suppress TradingView invalid-symbol rejections at the widget boundary.
...
If the task names product surfaces or routes, read the app repo's `AGENTS.md`, then resolve the current domain docs with `rg --files doc/domain-knowledge`. Older guidance may mention `doc/knowledge/glossary.md`; if that file is absent, use the relevant module's `domain-invariants.md` and `functionality.md` glossary section (for example `doc/domain-knowledge/rank/` or `doc/domain-knowledge/option-trades/`) and record the path drift in **EvidenceAudit**.
...
| --- | --- | --- | --- | --- |
| **Contract Rank direct snapshot recovered-fallback diagnostics + server fallback type repair** — `contractFlowRankService` splits direct JSON parse vs fetch/decode stages and marks direct snapshot/meta fallbacks with valid `failureKind=transient` plus `fallbackOutcome=recovered_fallback` / `fallbackTarget`, while the server fallback query casts index proxy volume denominators to a single `Float64` type so `symbol_context AS sc` cannot fail when CF direct falls back. | `tradingflow-webapp-fullstack` local patch, pending webapp deploy, updated 2026-06-24. Current checkout accepts the live `contract_rank_snapshot_compact_v9` direct payload and has `toFloat64(...)` casts in `src/server/services/contractFlowRankQueryService.ts`; production still needs post-deploy confirmation. | 2026-06-17 pre-fix: PostHog issue `019eb693-5781-7bd3-8a42-405e7854ce61` “CF snapshot returned unsupported compact payload” = 4 occurrences / 1 user; Better Stack pattern `dde126aeb830b9ee827a61e475f77e364c2aeed30f53a2cdde735e0688bef63c` = 2 exceptions; JSON parse issue `019e8dfa-d541-7132-a2cf-8a67347950c5` and Better Stack pattern `e32fc0da3a7021e0f7e3205f3bbaaf3e864a73806e8e330cd45c58007347299e` = 3-6 events; Better Stack Info `contractFlowRank.snapshot` direct snapshot failed `error.P1` = 7 events; `query:contractFlowRankSnapshot` unsupported payload `error.P0` = 2 events. 2026-06-24 verification evidence: live app version `0.2.1+2c9f8a9`; live direct snapshot probe returned `v=contract_rank_snapshot_compact_v9`, `rc=349795`, `rc == r.length`, first tuple length `19`; old PostHog issues `019eb693-5781-7bd3-8a42-405e7854ce61` / `019e8dfa-d541-7132-a2cf-8a67347950c5` had 0 sampled events; old Better Stack patterns `dde126a...` / `e32fc0...` = 0; new handled direct parse issue `019ef5de-fc1f-7720-9127-da5111fdd55e` = 1 event from `/assets/index-k71llIss.js`; Better Stack Info showed `contractFlowRank.snapshot` direct fallback P1 `fallbackOutcome=recovered_fallback` plus P0 server fallback failures. Server fallback baseline to clear: Better Stack patterns `78863cf63a913ee354d2bddcae0338343685389d6c3ae1c0ddfce3641f8a26c3` = 6 and `62b0379041ad6e4147f4beda3dd4bc163d5c889326490b7c08bfaa79a04e0b23` = 5 in `-24h`, message `There is no supertype for types UInt64, UInt32, Float64 ... In scope symbol_context AS sc`; Better Stack Telemetry `scope=contractFlowRank.snapshot` P0 = 5. | After deploy, query PostHog issues `019eb693-5781-7bd3-8a42-405e7854ce61`, `019e8dfa-d541-7132-a2cf-8a67347950c5`, and `019ef5de-fc1f-7720-9127-da5111fdd55e`; Better Stack patterns `dde126aeb830b9ee827a61e475f77e364c2aeed30f53a2cdde735e0688bef63c`, `e32fc0da3a7021e0f7e3205f3bbaaf3e864a73806e8e330cd45c58007347299e`, `78863cf63a913ee354d2bddcae0338343685389d6c3ae1c0ddfce3641f8a26c3`, and `62b0379041ad6e4147f4beda3dd4bc163d5c889326490b7c08bfaa79a04e0b23`; Better Stack Info for `scope=contractFlowRank.snapshot`, `failureKind=transient`, `fallbackOutcome=recovered_fallback`, `fallbackTarget in (server_snapshot, server_meta)`, `requestStage in (cf_direct_snapshot_parse, cf_direct_snapshot_decode, cf_direct_snapshot_meta_parse, cf_direct_snapshot_meta_decode, cf_direct_snapshot)`, and confirm no paired `contractFlowRank.snapshot` P0 or `query:contractFlowRankSnapshot` P0 for fallback attempts. Sanity-probe the producer shape with `curl -sS --max-time 20 https://ws.optiondata.io/api/v1/contract-rank/latest-snapshot` and confirm `v=contract_rank_snapshot_compact_v9`, `rc == r.length`, and the first tuple length is `19`. | No |
| **Netlify missing-asset 404 handling** — hashed `/assets/*` misses return a 404 page before the SPA shell catch-all so stale tabs see a real module fetch miss instead of `200 text/html` from `_shell.html`, while existing immutable assets still file-shadow and serve normally. | `tradingflow-webapp-fullstack` local patch, pending webapp deploy, 2026-06-24; implemented in `netlify.toml`, `public/404.html`, and `src/config/netlifyRedirects.test.ts`. | 2026-06-24 pre-fix: PostHog dynamic-import issue `019d2051-6a33-7b10-bb1e-54fa08bb3be7` = 79 occurrences / 5 users / 24 sessions; Better Stack `network error` pattern `2403c9ba5deee2af387834e95461cfb32b9b5080b21d6f307b2f09bb09e71f21` = 38; module/MIME pattern `bee015b18aa94302195d537637663a145195bd4771a28f57019eb929f0c3dd54` = 1; dynamic-import patterns `b2ded0728498c7cc53bf7745effd5215cbde6142d02306b04a1fa61721cc7a45`, `cebf6649295a2389b201e641cb6bc0e6e2e2a9cebb586f8bb4dff289862e9e2c`, and `ec9a3469f8ae71a89124119d047727cf2b34b326b4b039bbc9ff1fd2ddb9c74f` = 1 each. Live probes for stale `/assets/gamma-exposure-screener.lazy-DI28ZLQ0.js`, `/assets/index-D5u4BulH.js`, and `/assets/cookbooks.index-GFu0Bycw.js` returned `HTTP/2 200` with `content-type: text/html`. | After deploy, run `curl -I` against the stale asset URLs above and confirm HTTP status is `404` rather than `200`, with no SPA shell rewrite. Also query PostHog issue `019d2051-6a33-7b10-bb1e-54fa08bb3be7` and Better Stack patterns `2403c9ba5deee2af387834e95461cfb32b9b5080b21d6f307b2f09bb09e71f21`, `bee015b18aa94302195d537637663a145195bd4771a28f57019eb929f0c3dd54`, `b2ded0728498c7cc53bf7745effd5215cbde6142d02306b04a1fa61721cc7a45`, `cebf6649295a2389b201e641cb6bc0e6e2e2a9cebb586f8bb4dff289862e9e2c`, and `ec9a3469f8ae71a89124119d047727cf2b34b326b4b039bbc9ff1fd2ddb9c74f`; expect material drop, allowing a short tail from already-open stale sessions. | No |
| **Option Trades access-tiered fetch request + timeout diagnostics** — `useOptionTradesStore.fetchData` coerces unpaid rows and stats requests to the canonical public preview request at the shared fetch boundary, carries the access tier through direct fetch, cold-load, live snapshot, watchlist-change, refresh, and retry paths, and adds client timeout drift / visibility / online diagnostics for rows and stats timeout events. | `tradingflow-webapp-fullstack` commit `fcfa6e489`, merged into local `test` by `ee8ab2593`, pending webapp deploy, 2026-06-30; implemented in `src/stores/useOptionTradesStore.ts`, `src/pages/optionTrades/index.tsx`, `src/pages/optionTrades/hooks/useOptionTradesSession.ts`, and `src/pages/optionTrades/hooks/useLiveSessionReconciler.ts`. | 2026-06-30 pre-fix: Better Stack Telemetry `fetchOptionTradesRows` P0 `requestStage=timeout_client` rows timeout rows = 8; `fetchOptionTradesStats` P1 timeout = 1; `data.fetchOptionTradesStats` backend P0 `requestStage=access_guard`, message `Failed to fetch option trades stats`, `errorMessage=Paid access required`, pattern `40f926ceec27f0789c6f75b5997b85585f3c8784fb9f59d2d1180f13df6a08b8` = 1 from a free user on `/app/option-trades/historical`; PostHog issue `019f11d7-6eff-7b02-b547-fdd21dff8478` title `Stats request timed out after 25s` had 7 occurrences / 2 users / 2 sessions, but sampled events carried `Rows request timed out after 22s` on `/app/rank/contracts`. 2026-06-30 rerun: production still `0.2.1+b85fabd`, so fix remains unverified; Better Stack access-guard pattern/logs = 0, but rows timeout logs = 6 and Better Stack timeout pattern `358ea88303e1eb9fc5ad2b06c1b7cfceff979e4da75c75b1b85d1c928c3aea6f` = 3. | After deploy, query Better Stack Errors pattern `40f926ceec27f0789c6f75b5997b85585f3c8784fb9f59d2d1180f13df6a08b8` and Better Stack Info for `surface=option-trades`, `operation=fetchOptionTradesStats`, `requestStage=access_guard`, `errorMessage="Paid access required"`; expected `0` for free-user stats access-guard regressions. Query `surface=option-trades`, `operation in (fetchOptionTradesRows, fetchOptionTradesStats)`, `requestStage=timeout_client`; new rows may still exist, but should include `clientTimeoutMs`, `timeoutDriftMs`, `documentVisibilityState`, `documentHidden`, `navigatorOnline`, `accessTier`, and `requestCoercedToPreview` fields for post-deploy diagnostics. Re-check PostHog issue `019f11d7-6eff-7b02-b547-fdd21dff8478` event `value` before classifying rows vs stats. | No |
