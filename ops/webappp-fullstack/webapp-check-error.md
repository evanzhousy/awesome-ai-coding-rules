...
Active backlog is not empty. Start every real run with [Active fix verification backlog](#active-fix-verification-backlog); current unconfirmed rows remain Contract Rank direct snapshot/server fallback verification, Netlify missing-asset 404 handling, and TradingView ticker-tape invalid-symbol diagnostics. 2026-07-01 rerun evidence used production `0.2.1+78e6938`, built `2026-07-01T06:07:38.468Z`. Option Trades access-tier/timeout diagnostics passed and were archived: access-guard `Paid access required` regressions were 0, stats timeouts were 0, rows timeout logs were 4 in `-24h` but only the post-deploy row included the expected diagnostics (`clientTimeoutMs=22000`, `timeoutDriftMs=5738777`, `documentVisibilityState=hidden`, `documentHidden=true`, `navigatorOnline=true`, `accessTier=free`, `requestCoercedToPreview=true`). Netlify asset probes for `/assets/gamma-exposure-screener.lazy-DI28ZLQ0.js`, `/assets/index-D5u4BulH.js`, `/assets/cookbooks.index-GFu0Bycw.js`, and `/assets/contract-rank.lazy-HJ_C0EY8.js` returned `404 text/html`, so the routing behavior is deployed; do not archive the Netlify row yet because PostHog issue `019d2051-6a33-7b10-bb1e-54fa08bb3be7` still had 34 occurrences / 6 users / 17 sessions and Better Stack pattern `2403c9ba5deee2af387834e95461cfb32b9b5080b21d6f307b2f09bb09e71f21` still showed `network error` = 29 in `-24h`. The live CF snapshot endpoint returned valid `contract_rank_snapshot_compact_v9` shape (`d=2026-07-01`, `rc=340133`, `rc == r.length`, first tuple length `19`, valid `u`/`g` maps, 43.5 MB payload), old unsupported-payload/type-repair patterns were 0, and PostHog search for `contractFlowRankSnapshot` returned 0; do not archive the Contract Rank row yet because Better Stack Info still showed `contractFlowRank.snapshot` P1 recovered fallbacks and `query:contractFlowRankSnapshot` P0 inactivity-timeout failures. TradingView invalid-symbol noise is no longer silent; a 2026-07-01 code fix canonicalizes NDX aliases (`NDXP` / `XND` -> `NASDAQ:NDX`) and adds paired `configuredSymbolMappings` diagnostics for future third-party ticker-tape invalid-symbol rows. After deploy, verify PostHog issues `019f1b9d-898d-7cf1-8495-26f0df9b9a6a` / `019f1f0c-2a0f-7220-ba50-58e306f540f3` and Better Stack patterns `aa7332f87002488125e24c84372c7b27bb2989e31c94d2daf815b70f29d64599` / `bb0f6afe12034784f3dd15895acb0c584699730a2fbf1dc97219674185b069a2`.
...
| **TradingView ticker-tape invalid-symbol canonicalization + paired diagnostics** — shared TradingView symbol resolution canonicalizes NDX alias roots (`NDXP`, `XND`) to `NASDAQ:NDX` before widget config and ticker-tape third-party invalid-symbol telemetry includes `configuredSymbolMappings` so raw watchlist symbols are paired with the generated TradingView symbol. | `tradingflow-webapp-fullstack` local patch, pending webapp deploy, 2026-07-01; implemented in `src/lib/tradingview/symbols.ts`, `src/pages/components/ticker-tape/mapWatchlistToTradingViewTape.ts`, `src/pages/components/ticker-tape/index.tsx`, and `src/lib/tradingview/observability.ts`. | 2026-07-01 pre-fix: PostHog issue `019f1b9d-898d-7cf1-8495-26f0df9b9a6a` (`TradingView widget rejected a symbol`) = 2 occurrences / 2 users / 2 sessions; PostHog issue `019f1f0c-2a0f-7220-ba50-58e306f540f3` = 1 occurrence / 1 user / 1 session; Better Stack patterns `aa7332f87002488125e24c84372c7b27bb2989e31c94d2daf815b70f29d64599` = 2 and `bb0f6afe12034784f3dd15895acb0c584699730a2fbf1dc97219674185b069a2` = 1. Better Stack Info rows had `surface=tradingview`, `operation=renderTradingViewWidget`, `requestStage=symbol_validation`, `thirdPartyErrorMessage='[tv] invalid symbol'`, and `configuredTradingViewSymbols` containing `NASDAQ:NDXP`, but `rawSymbol` was blank for ticker-tape third-party rows. | After deploy, query the PostHog issues and Better Stack patterns above. Expected: `NASDAQ:NDXP` no longer appears in `configuredTradingViewSymbols`; `configuredSymbolMappings` includes paired entries such as `NDXP=>NDX=>NASDAQ:NDX`; new `TickerTapeWidget` third-party invalid-symbol rows should either be 0 or include `configuredSymbolMappings` so the raw offender can be identified. Also run a targeted Better Stack Info query for `surface=tradingview`, `operation=renderTradingViewWidget`, `requestStage=symbol_validation`, and confirm no row has blank `rawSymbol` without a non-empty `configuredSymbolMappings` field. | No |
