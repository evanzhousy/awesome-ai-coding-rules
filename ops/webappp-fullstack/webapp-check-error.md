...
Active backlog is not empty. Start every real run with [Active fix verification backlog](#active-fix-verification-backlog); no row was archived on the 2026-07-21 rerun because Better Stack tools were unavailable in both delegated evidence runtimes. PostHog project `300646` (`app.tradingflow.com`) was confirmed for `2026-07-20T06:48:11Z` through `2026-07-21T06:48:11Z`; the live app was `0.2.1+50d1192`, built `2026-07-20T18:46:49.279Z`, and that commit contains the Rank fallback hardening. Contract Rank remains **pending**: the only new timeout issue found (`019f7c70-7428-7791-a759-f49e140bf7f6`, 1 occurrence / 1 user / 1 session) was pre-deploy, the post-deploy PostHog snapshot search was empty, and the producer returned valid `contract_rank_snapshot_compact_v9` (`d=2026-07-20`, `rc=333965`, `rc == r.length`, first tuple length `19`), but Better Stack pattern and paired-P0 checks were blocked. Asset-miss routing remains deployed across Vercel/Netlify and all four stale asset probes returned `404 text/html`, while PostHog issue `019d2051-6a33-7b10-bb1e-54fa08bb3be7` still had 33 occurrences / 3 users / 17 sessions; keep the row pending until Better Stack is checked and the stale-session tail decays. TradingView remains **pending**: legacy issue `019f3430-50f2-7201-bc2c-eaa20b6744a0` was absent from the active list, but new structured issue `019f7c34-b504-7560-84dd-a9c63fc47738` had 4 occurrences / 3 users / 4 sessions including post-deploy activity, and the raw duplicate pattern could not be checked. The net-new missing production `FEATUREBASE_JWT_SECRET` finding was fixed operationally on 2026-07-21: the sensitive Vercel Production value was overwritten from the established local source and production deployment `dpl_EL1krcX6jAD6eNvzwdKxuKnuyqVq` was redeployed as `dpl_3p9c1JNzYcHWzbmmkPLBMEzpSGxT` (same commit `50d1192`, built `2026-07-21T10:16:52.324Z`). Immediate post-deploy checks showed no matching Vercel errors, zero PostHog impact for both Featurebase issues, and zero Better Stack `FEATUREBASE_JWT_SECRET` / `Server env validation failed` errors; keep the new backlog row pending for a full-window confirmation.
...
| --- | --- | --- | --- | --- |
| **Contract Rank direct snapshot recovered-fallback diagnostics + server fallback type repair** — `contractFlowRankService` splits direct JSON parse vs fetch/decode stages and marks direct snapshot/meta fallbacks with valid `failureKind=transient` plus `fallbackOutcome=recovered_fallback` / `fallbackTarget`, while the server fallback query casts index proxy volume denominators to a single `Float64` type so `symbol_context AS sc` cannot fail when CF direct falls back. | `tradingflow-webapp-fullstack` patch is deployed in production `0.2.1+78e6938` as of 2026-07-01, but this row remains unconfirmed because current verification still shows paired P0 fallback failures. Current checkout accepts the live `contract_rank_snapshot_compact_v9` direct payload and has `toFloat64(...)` casts in `src/server/services/contractFlowRankQueryService.ts`. | 2026-06-17 pre-fix: PostHog issue `019eb693-5781-7bd3-8a42-405e7854ce61` “CF snapshot returned unsupported compact payload” = 4 occurrences / 1 user; Better Stack pattern `dde126aeb830b9ee827a61e475f77e364c2aeed30f53a2cdde735e0688bef63c` = 2 exceptions; JSON parse issue `019e8dfa-d541-7132-a2cf-8a67347950c5` and Better Stack pattern `e32fc0da3a7021e0f7e3205f3bbaaf3e864a73806e8e330cd45c58007347299e` = 3-6 events; Better Stack Info `contractFlowRank.snapshot` direct snapshot failed `error.P1` = 7 events; `query:contractFlowRankSnapshot` unsupported payload `error.P0` = 2 events. 2026-06-24 verification evidence: live app version `0.2.1+2c9f8a9`; live direct snapshot probe returned `v=contract_rank_snapshot_compact_v9`, `rc=349795`, `rc == r.length`, first tuple length `19`; old PostHog issues `019eb693-5781-7bd3-8a42-405e7854ce61` / `019e8dfa-d541-7132-a2cf-8a67347950c5` had 0 sampled events; old Better Stack patterns `dde126a...` / `e32fc0...` = 0; new handled direct parse issue `019ef5de-fc1f-7720-9127-da5111fdd55e` = 1 event from `/assets/index-k71llIss.js`; Better Stack Info showed `contractFlowRank.snapshot` direct fallback P1 `fallbackOutcome=recovered_fallback` plus P0 server fallback failures. Server fallback baseline to clear: Better Stack patterns `78863cf63a913ee354d2bddcae0338343685389d6c3ae1c0ddfce3641f8a26c3` = 6 and `62b0379041ad6e4147f4beda3dd4bc163d5c889326490b7c08bfaa79a04e0b23` = 5 in `-24h`, message `There is no supertype for types UInt64, UInt32, Float64 ... In scope symbol_context AS sc`; Better Stack Telemetry `scope=contractFlowRank.snapshot` P0 = 5. | Query PostHog issues `019eb693-5781-7bd3-8a42-405e7854ce61`, `019e8dfa-d541-7132-a2cf-8a67347950c5`, and `019ef5de-fc1f-7720-9127-da5111fdd55e`; Better Stack patterns `dde126aeb830b9ee827a61e475f77e364c2aeed30f53a2cdde735e0688bef63c`, `e32fc0da3a7021e0f7e3205f3bbaaf3e864a73806e8e330cd45c58007347299e`, `78863cf63a913ee354d2bddcae0338343685389d6c3ae1c0ddfce3641f8a26c3`, and `62b0379041ad6e4147f4beda3dd4bc163d5c889326490b7c08bfaa79a04e0b23`; Better Stack Info for `scope=contractFlowRank.snapshot`, `failureKind=transient`, `fallbackOutcome=recovered_fallback`, `fallbackTarget in (server_snapshot, server_meta)`, `requestStage in (cf_direct_snapshot_parse, cf_direct_snapshot_decode, cf_direct_snapshot_meta_parse, cf_direct_snapshot_meta_decode, cf_direct_snapshot)`, and confirm no paired `contractFlowRank.snapshot` P0 or `query:contractFlowRankSnapshot` P0 for fallback attempts. Sanity-probe the producer shape with `curl -sS --max-time 20 https://ws.optiondata.io/api/v1/contract-rank/latest-snapshot` and confirm `v=contract_rank_snapshot_compact_v9`, `rc == r.length`, and the first tuple length is `19`. | No |
| **Cross-platform missing-asset 404 handling** — hashed `/assets/*` misses return a 404 page before the SPA shell catch-all so stale tabs see a real module fetch miss instead of `200 text/html` from `_shell.html`, while existing immutable assets still file-shadow and serve normally. | The Netlify path is implemented in `netlify.toml`, `public/404.html`, and `src/config/netlifyRedirects.test.ts`; the current Vercel production path is implemented in `scripts/vercel-output-routes.mjs`, `scripts/patch-vercel-output.mjs`, and `src/config/vercelRoutes.test.ts` (asset-miss fix `e82456431`, included in current production). | 2026-06-24 pre-fix: PostHog dynamic-import issue `019d2051-6a33-7b10-bb1e-54fa08bb3be7` = 79 occurrences / 5 users / 24 sessions; Better Stack `network error` pattern `2403c9ba5deee2af387834e95461cfb32b9b5080b21d6f307b2f09bb09e71f21` = 38; module/MIME pattern `bee015b18aa94302195d537637663a145195bd4771a28f57019eb929f0c3dd54` = 1; dynamic-import patterns `b2ded0728498c7cc53bf7745effd5215cbde6142d02306b04a1fa61721cc7a45`, `cebf6649295a2389b201e641cb6bc0e6e2e2a9cebb586f8bb4dff289862e9e2c`, and `ec9a3469f8ae71a89124119d047727cf2b34b326b4b039bbc9ff1fd2ddb9c74f` = 1 each. Live probes for stale `/assets/gamma-exposure-screener.lazy-DI28ZLQ0.js`, `/assets/index-D5u4BulH.js`, and `/assets/cookbooks.index-GFu0Bycw.js` returned `HTTP/2 200` with `content-type: text/html`. | Run `curl -I` against the stale asset URLs above plus `/assets/contract-rank.lazy-HJ_C0EY8.js` and confirm HTTP status is `404` rather than `200`, with no SPA shell rewrite. Also query PostHog issue `019d2051-6a33-7b10-bb1e-54fa08bb3be7` and Better Stack patterns `2403c9ba5deee2af387834e95461cfb32b9b5080b21d6f307b2f09bb09e71f21`, `bee015b18aa94302195d537637663a145195bd4771a28f57019eb929f0c3dd54`, `b2ded0728498c7cc53bf7745effd5215cbde6142d02306b04a1fa61721cc7a45`, `cebf6649295a2389b201e641cb6bc0e6e2e2a9cebb586f8bb4dff289862e9e2c`, and `ec9a3469f8ae71a89124119d047727cf2b34b326b4b039bbc9ff1fd2ddb9c74f`; expect material drop, allowing a short tail from already-open stale sessions. | No |
| **TradingView ticker-tape duplicate raw vendor invalid-symbol suppression** — ticker-tape captures third-party `[tv] invalid symbol` window errors as one structured app `error.P1`, suppresses the raw browser `error` / `unhandledrejection` event after reporting, and parses `thirdPartyReportedSymbol` when the vendor message includes one. | `tradingflow-webapp-fullstack` commit `453548da2` on `test`, pending deploy, 2026-07-08; implemented in `src/lib/tradingview/observability.ts`, `src/pages/components/ticker-tape/index.tsx`, and focused tests. | 2026-07-08 pre-fix: PostHog issue `019f3430-50f2-7201-bc2c-eaa20b6744a0` (`TradingView widget rejected a symbol`) = 5 occurrences / 4 users / 4 sessions on `/app/option-trades/live` and `/app/rank/symbols`; Better Stack structured pattern `002542d631afc57ddc5d8034f2a34eb92e307ed39bb9b4b810fa091f69827999` = 4; duplicate raw browser pattern `904676ed82ffa77159cb99b64bc737ecd6063e8021d7cdb6a7004bbc402ebc1c` (`Non-Error promise rejection captured with value: [tv] invalid symbol`) = 4. Better Stack Info rows had `surface=tradingview`, `operation=renderTradingViewWidget`, `requestStage=symbol_validation`, `configuredSymbolMappings` for `TSLA`, `AAPL`, `META`, `AMZN`, `MSFT`, `NVDA`, and blank `rawSymbol` because the vendor did not name the exact offender. | After deploy, query PostHog issue `019f3430-50f2-7201-bc2c-eaa20b6744a0` and Better Stack patterns `002542d631afc57ddc5d8034f2a34eb92e307ed39bb9b4b810fa091f69827999` / `904676ed82ffa77159cb99b64bc737ecd6063e8021d7cdb6a7004bbc402ebc1c`. Expected: raw unhandled pattern `904676ed...` drops to 0 or materially disappears; structured pattern `002542d...` is either 0 or remains as the single P1 diagnostic path. Better Stack Info for `surface=tradingview`, `operation=renderTradingViewWidget`, `requestStage=symbol_validation` should still include `configuredSymbolMappings`; if TradingView names an offender, `thirdPartyReportedSymbol` should be non-empty. | No |
| **Featurebase production identity secret** — production Featurebase widgets retain their required server-minted identity boundary; the fix restores the deployment-owned signing secret instead of weakening the fail-closed code path. | Vercel Production `FEATUREBASE_JWT_SECRET` overwritten as a sensitive value and production deployment `dpl_EL1krcX6jAD6eNvzwdKxuKnuyqVq` redeployed as `dpl_3p9c1JNzYcHWzbmmkPLBMEzpSGxT` on 2026-07-21; same app commit `50d1192`, build time `2026-07-21T10:16:52.324Z`. | 2026-07-21 pre-fix: PostHog runtime issue `019f6e7f-02f3-7dd1-82a3-12167d49bba8` = 3 occurrences / 1 user and prerender issue `019f6e7d-1350-7420-b31f-b8601ef8e438` = 1 occurrence / 1 user around the prior deploy. Immediate post-fix checks found zero impact for both issues after `2026-07-21T10:16:23Z`, zero Better Stack errors matching `FEATUREBASE_JWT_SECRET` or `Server env validation failed`, and no matching Vercel runtime logs. | Query both PostHog issue UUIDs and Better Stack Errors application resolved at run time for `FEATUREBASE_JWT_SECRET`, `featurebase.getFeaturebaseJwt`, `featurebase.identityJwt`, and `server.envValidation` over a full post-deploy window. Confirm an authenticated production session reaches Featurebase `identified` state and `/user/product-feedback` loads without a blank/error state. Expected: zero missing-secret/env-validation errors; any unrelated Featurebase vendor/runtime issue is triaged separately. | No |
