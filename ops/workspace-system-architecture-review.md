...
Latest run: a local-repo-only architecture review was executed on 2026-06-24 across the default projects present on disk: `awesome-ai-coding-rules`, `tradingflow-webapp-fullstack`, `tradingflow-cfworker-service`, `tradingflow-process-service-ec2`, `tradingflow-web-landingpage`, and `optiondata-portal`. No production, browser, observability, billing, or database probes were run, and no sibling production systems were mutated.
...
Secondary repos named by ops but not present in the local workspace during that run were `api-service-lambda`, `cron-service-lambda`, and `quant-service`; treat them as out of scope unless a current checkout or live evidence shows they still own an active capability.
